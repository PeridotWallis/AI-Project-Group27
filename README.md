...
If the release page still shows only source-code archives, push the latest changes to `main`, then open the repository's **Actions** tab, select **Build and publish desktop installers**, choose **Run workflow** on the `main` branch, and enter `v1.0.0` as the tag. The manual run builds the selected branch and attaches its installers to the tag you entered. Wait for the workflow to finish successfully; the installer files are attached only after the build jobs succeed.
